...
export default function Seguindo({ idPerfil, arroba }) {
  ...
  async function desseguir(idUserPerfil) {
    const idUserSalvo = await AsyncStorage.getItem('idUser');
    result = await axios.get(`http://localhost:8000/api/cursei/user/deseguirOuTirarSeguidor/${idUserSalvo}/${idUserPerfil}/desseguir`)
  ...
  useEffect(() => {
      ...
      url = `http://localhost:8000/api/cursei/user/seguidoresSeguindo/${idPerfil}/seguindo`
    ...
    async function pesquisar(text) {
      ...
      setLoading(true)
      response = await axios.get(`http://localhost:8000/api/cursei/user/deseguirOuTirarSeguidor/${text}/${idPerfil}/pesquisarSeguindo`)
      setLoading(false)
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ flexGrow: 1 }}
      ...
      <Text style={styles.titulo}>Pessoas seguidas por @{arroba} ({usuarios?.length || 0})</Text>
