...
export default function Seguindo({ idPerfil, arroba }) {
  ...
  async function desseguir(idUserPerfil) {
    ...
    result = await axios.get(`http://${host}:8000/api/cursei/user/deseguirOuTirarSeguidor/${idUserSalvo}/${idUserPerfil}/desseguir`)
  ...
  useEffect(() => {
      ...
      url = `http://${host}:8000/api/cursei/user/seguidoresSeguindo/${idPerfil}/seguindo`
    ...
    async function pesquisar(text) {
      ...
      setLoading(true)
      response = await axios.get(`http://${host}:8000/api/cursei/user/deseguirOuTirarSeguidor/${text}/${idPerfil}/pesquisarSeguindo`)
      setLoading(false)
